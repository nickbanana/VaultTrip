...
object VaultScanner {
    ...
    fun setIgnoreUri(ignoreUri: Uri?) {
        ...
        try {
            ...
            if (treeDocId != null) {
                ...
                Log.d("VaultScanner", "Set ignore target. URI: $ignoreUri (treated as tree) has DocId: $documentIdToIgnore")
            } else {
                // If not a tree, try as a regular document URI
                val docId = DocumentsContract.getDocumentId(ignoreUri)
                documentIdToIgnore = docId
                Log.d("VaultScanner", "Set ignore target. URI: $ignoreUri (treated as document) has DocId: $documentIdToIgnore")
            }
        } catch (e: IllegalArgumentException) {
            Log.w("VaultScanner", "ignoreUri $ignoreUri is not a valid tree or document URI to get a documentId from. Error: ${e.message}")
            documentIdToIgnore = null
        }
    }
    ...
    private fun fetchAllMarkdownFilesRecursive(
        ...
        if (
            ...
            Log.d("FetchMarkdown", "Skipping ignored directory by Document ID: $currentParentDocumentId in tree $treeUri")
            return
        }
        ...
        val currentDirectoryUri = DocumentsContract.buildDocumentUriUsingTree( // For logging or other purposes
            treeUri,
            currentParentDocumentId
        )
        Log.d("FetchMarkdown", "Processing directory: $currentDirectoryUri (DocID: $currentParentDocumentId within Tree: $treeUri)")
            ...
            context.contentResolver.query(childrenUri, projection, null, null, null)?.use { cursor ->
                while (cursor.moveToNext()) {
                    ...
                    if (docIdIndex == -1 || nameIndex == -1 || mimeTypeIndex == -1) {
                        Log.w("FetchMarkdown", "Required column not found in cursor for $childrenUri")
                        continue
                    }
                    ...
                    if (mimeType == DocumentsContract.Document.MIME_TYPE_DIR) {
                        ...
                        fetchAllMarkdownFilesRecursive(context, treeUri, docId, allMarkdownFiles)
                        ...
                        Log.d("FetchMarkdown", "Found Markdown file: ${documentItem.name}, Uri: ${documentItem.itemUri}")
                    }
                }
            }
        } catch (e: Exception) {
            // Catch potential SecurityExceptions or other issues during query
            Log.e("FetchMarkdown", "Error querying children for $currentParentDocumentId under $treeUri", e)
     ...
     */
    fun getAllMarkdownFilesFromUri(context: Context, rootUri: Uri, ignoreUri: Uri?): List<DocumentItem> {
        ...
        try {
            ...
            Log.e("FetchMarkdown", "Failed to ensure persistable read permission for $rootUri. Cannot fetch files.", e)
            Toast.makeText(context, "Permission error for $rootUri. Cannot scan files.", Toast.LENGTH_LONG).show()
        ...
        if (rootDocId == null) {
            ...
            Toast.makeText(context, "Could not access root URI: $rootUri", Toast.LENGTH_SHORT).show()
            return emptyList()
        }
        ...
        if (documentIdToIgnore != null && rootDocId == documentIdToIgnore) {
            Log.d("FetchMarkdown", "Root URI is the ignored directory. Skipping entire scan based on Document ID.")
            return emptyList()
        }

        val markdownFiles = mutableListOf<DocumentItem>()
        Log.d("FetchMarkdown", "Starting recursive search from root: $rootUri, rootDocId: $rootDocId")
